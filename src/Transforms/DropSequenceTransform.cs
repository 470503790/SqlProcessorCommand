...
{
    /// <summary>
    /// 处理语句：<c>DROP SEQUENCE [schema].[sequence_name]</c><br/>
    /// - 通过 <c>OBJECT_ID('[schema].[sequence_name]', 'SO')</c> 判断序列是否存在；<br/>
    /// - 存在时才 DROP。适用于 SQL Server 2012+ 的 SEQUENCE 对象。
    /// </summary>
    internal sealed class DropSequenceTransform : ISqlBlockTransform
    {
